"""src/apps/buildings/tests/test_houses.py."""
...
async def test_create_house_structure(client: AsyncClient, developer_headers):
    ...
    response = await client.post("/houses/", json=payload, headers=developer_headers)
...
async def test_user_cannot_create_house(client: AsyncClient, auth_headers):
    ...
    response = await client.post("/houses/", json=payload, headers=auth_headers)
